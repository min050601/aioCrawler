...
class H558TelSpiderSpider(Spider):
    name = 'h5_58_tel_spider'
    routing_key='h5_58_tel_spider_request'
        ...
        for k,v in self.city_dict.items():
            for city,code in v.items():
                for wtype in self.work_type:
                    ...
                    request=Request(url=url,method='GET',headers=headers,meta={'city':city,'page':1,'url':url},callback='parse',allow_redirects=False,allow_proxy=False)
    ...
    def parse(self,response):
        ...
        if response.status==200:
            ...
            items = selector.xpath("//li[@class='job_item clearfix']")
            for item in items:
                info_id = item.xpath("./a[@class='item_con apply']/@infoid").extract_first('')
                entname = item.xpath("./div[@class='item_con job_comp']/div[@class='comp_name']/a[@class='fl']/@title").extract_first('')
                insert_sql='''insert into h5_58_ent(entname,info_id) VALUES (%s,%s)'''
                self.Pipeline.process_item((insert_sql, (entname,info_id)))
            total_page = selector.xpath("//span[@class='total_page']/text()").extract_first('1')
            if int(total_page)>response.request.meta['page']:
                next_url=response.request.meta['url']+'pn%s/'%(response.request.meta['page']+1)
    ...
    work_type=['/zplvyoujiudian/','/jiazhengbaojiexin/','/meirongjianshen/','/jiudianzp/','/zpjiudian/','/zpwentiyingshi/','/zpanmo/','/zpjianshen/',
               '/renli/','/siji/','/zpguanli/','/yewu/','/kefu/','/zpshangwumaoyi/','/chaoshishangye/','/zptaobao/','/zpfangchan/','/shichang/','/zpguanggao/',
               '/zpmeishu/','/zpshengchankaifa/','/zpshengchan/','/zpwuliucangchu/','/xiaofeipin/','/zhikonganfang/','/zpqiche/','/tech/','/zpjixieyiqi/',
               '/zpjixie/','/zpzhiyepeixun/','/zpfalvzixun/','/zhuanye/','/fanyizhaopin/','/zpxiezuochuban/','/zpcaiwushenji/','/jinrongtouzi/','/zpjinrongbaoxian/',
               '/zpyiyuanyiliao/','/zpzhiyao/','/huanbao/','/zpfangchanjianzhu/','/zpwuye/','/nonglinmuyu/','/zhaopin/']
